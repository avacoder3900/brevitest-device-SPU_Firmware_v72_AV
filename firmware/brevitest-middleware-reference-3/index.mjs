...
const bcodeCommands = [{
    ...
    num: '16',
    name: 'SCAN SENSOR',
    params: ['baseline', 'starting_position', 'distance_to_scan', 'step_delay'],
...
const validate_cartridge = async (device, body) => {
    ...
    try {
        ...
        if (error.message) {
            response = await send_response(device.id, 'validate-cartridge', 'FAILURE', { cartridgeId, errorMessage: error.message });
        } else {
            response = await send_response(device.id, 'validate-cartridge', 'ERROR', { cartridgeId, errorMessage: error });
        }
    }
    return response;
};
...
const test_upload = async (device, body) => {
    ...
    try {
        ...
        if (cartridge.status === 'completed' || cartridge.status === 'cancelled') {
            ...
            updatedCartridge = await saveDocument(cartridge, cartridge.siteId || cartridge.department);
        }
        console.log('updatedCartridge', updatedCartridge);
